from stablethread.settings import AssistantSettings, Settings, SpeechSettings
from pydantic_settings import SettingsConfigDict


def test_api_key_initialization_from_env(env):
    test_api_key = "test_api_key_123"
    env.set("STABLETHREAD_OPENAI_API_KEY", test_api_key)

    temp_model_config = SettingsConfigDict(env_prefix="stablethread_")
...
def test_runtime_api_key_override(env):
    ...
    env.set("STABLETHREAD_OPENAI_API_KEY", override_api_key)
