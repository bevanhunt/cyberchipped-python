...
async def test_text(ai_instance, sqlite_db):
    ...
    async with aiosqlite.connect(sqlite_db) as db:
        async with db.execute(
            "SELECT * FROM messages WHERE user_id = ?", ("user_123",)
        ) as cursor:
...
async def test_get_current_temperature_tool(ai_instance):
    response = await ai_instance.text(
        "user_123", "What is the current temperature in New York, NY in Celsius?"
    )
    assert response is not None
...
async def test_get_rain_probability_tool(ai_instance):
    response = await ai_instance.text(
        "user_123", "What is the probability of rain in San Francisco, CA?"
    )
    assert response is not None
